---
name: git-worktree
description: 격리된 브랜치 폴더와 병렬 에이전트 세션을 위해 Git worktree를 생성, 진입, 목록화, 삭제, 정리, 복구할 때 사용하며, 이미 해당 linked worktree 안에 있는 상태에서 "워크트리 삭제"처럼 요청한 경우에도 안전하게 처리합니다. `.hypercore/git-worktree/<folder_name>` 프로젝트 규칙으로 워크트리를 만들거나 관리해 달라는 요청에 맞추며, 생성 작업의 목적이 불명확하면 먼저 어떤 작업을 할지 물어 폴더명을 정한 뒤 새 worktree로 후속 작업 컨텍스트를 이동합니다.
...
5. 생성 작업이고 작업 목적이 불명확하면 먼저 어떤 작업을 할 예정인지 한 문장으로 물어봅니다.
6. 답변 또는 기존 요청 맥락에서 `.hypercore/git-worktree/<folder_name>`의 폴더명을 정합니다.
7. 자세한 명령과 안전 규칙은 `@rules/worktree-lifecycle.md`를 따릅니다.
8. 생성 직후 후속 작업 컨텍스트를 새 worktree 경로로 이동하고, 삭제 후에는 제거된 경로와 남은 worktree 목록을 보고합니다. 경로, 브랜치/커밋, clean/dirty 상태, 진입·열기 명령, 남은 setup도 해당 작업에 맞게 보고합니다.
