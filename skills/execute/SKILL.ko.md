---
name: execute
description: 주어진 작업을 난이도에 맞는 적응형 사고 깊이로 즉시 수행한다. 진단, 계획, 리뷰가 아닌 즉각적인 실행이 필요할 때 사용.
...
- 전략 계획이나 아키텍처 결정 → `plan`.
- 코드 리뷰나 품질 감사 → `code-review`.
- 보안 분석 → `security-review`.
...
구현 전에 항상 `sequential-thinking`을 실행한다. 사고 횟수는 난이도에 비례:
