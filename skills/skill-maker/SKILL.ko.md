...
- 주요 스킬 생성이나 리팩토링 전에는 내부 구조화 사고 패스를 수행합니다. 외부 reasoning MCP나 전용 reasoning tool을 요구하지 않습니다.
- 리팩터가 지속 가능한 trace를 필요로 할 만큼 중요하면 구조 계획을 검증 메모에 기록합니다.
...
| 2 | 내부 구조화 사고 패스로 구조 계획 수립 | 섹션/자원 계획 |
