...
| 토큰 | 공유하는 스킬 |
|---|---|
| `refactor` | 이 스킬과 형제 산출물 작성 스킬 네 개를 포함한 5개 |
| `create` | 12개 |

따라서 이 스킬의 가장 가까운 이웃은 `refactor`를 공유하는 네 스킬이며, 공존 케이스는 재사용
가능한 *스킬 폴더*를 재사용 가능한 *프롬프트*, *문서*, *지침 파일*과 구분해야 합니다. 위 측정값은
개수만 적고 이웃 이름은 적지 않습니다. 규칙 파일이 이웃 이름을 적으면 그 파일만으로 쓸 수 없게
됩니다(`rules/self-containment.md`, SK-S-2).

## 8. 범위 경계

스킬이 소유하지 않는 것을 밝힙니다. 모든 경계는 그 산출물을 소유한 스킬의 이름이 아니라 산출물의
형태로 서술합니다(`rules/self-containment.md`, SK-S-2).

예:

- 산출물이 스킬 폴더이거나 스킬 리팩터링이면 이 스킬이 소유합니다
- 일반 문서, 런북, 가이드는 범위 밖입니다
- 재사용 가능한 프롬프트 산출물은 범위 밖입니다
- 출처 기반 사실 확인은 범위 밖입니다
- 구현 전 계획은 범위 밖입니다
- 커밋 생성은 범위 밖입니다
