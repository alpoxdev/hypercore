...
이 스킬은 실제로 Codex CLI나 기존 Codex 세션이 필요한 요청에만 사용한다.
...
- Codex CLI 자체가 필요하지 않은 일반 문서 작성, 런북 정리, 스킬 생성은 다른 스킬이나 직접 편집으로 전환한다.

## 작업 실행

샌드박스를 바꾸거나 세션을 재개하기 전에는 [references/recipes.ko.md](/Users/alpox/Desktop/dev/kood/hypercore/skills/codex/references/recipes.ko.md)의 구체적인 명령 예시를 먼저 본다.
