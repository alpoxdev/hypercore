...
이 스킬은 단독으로도 바로 실행 가능해야 합니다. 이미 계획 또는 실행 워크플로우가 활성화되어 있다면, 외부 프롬프트 표면을 강제하지 말고 이 아키텍처 게이트를 그 워크플로우 안으로 가져가세요.
...
- `rules/conventions.ko.md` - 코드 컨벤션 (네이밍, TypeScript, 임포트, 주석)
- `rules/routes.ko.md` - 라우트 구조 (폴더 규칙, 패턴, 로더)
- `rules/services.ko.md` - API 서비스 (스키마, 쿼리, 뮤테이션)
- `rules/hooks.ko.md` - 커스텀 훅 패턴 (분리 규칙, 내부 순서)
...
계획 기반으로 실행 중이라면, 현재 작업의 인수 기준에 다음 항목을 포함하세요:
