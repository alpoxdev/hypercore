...
| `routeTree.gen.ts` is generated by Start/Router tooling and SHOULD be committed to git | Official + Safety policy | Do not hand-edit for normal architecture work; do commit the generated file |
...
- `src/routeTree.gen.ts` is generated. Do not manually rewrite it to change routes, and commit it: the official FAQ answers "Should I commit my `routeTree.gen.ts` file into git?" with "Yes!", because the generated route tree is "part of your application's runtime, not a build artifact" (Official).
...
The example above uses Start's Vite plugin (`@tanstack/react-start/plugin/vite`). Vite is one of two official bundler integrations; Rsbuild is the other, with its own plugin entry `@tanstack/react-start/plugin/rsbuild` (Official). Use the plugin path that matches the project's bundler instead of assuming Vite.
...
This shape is a Hypercore convention. TanStack Router officially supports flat, directory, and mixed route structures, so do not call flat route files invalid official TanStack usage. Router's default `routeFileIgnorePrefix` is `-` (Official), so `-components`, `-hooks`, and `-functions` are appropriate co-located folders that are not treated as route files. That official default is what legitimizes this skill's `-hooks/`, `-components/`, and `-functions/` convention: the prefix, not a Hypercore invention, is what keeps those folders out of the route tree. The same docs warn not to set `routeFilePrefix`, `routeFileIgnorePrefix`, or `routeFileIgnorePattern` to match any token used in the File Naming Conventions guide, or unexpected behavior may follow (Official).
...
File naming rules (kebab-case filenames, hook and component naming, TypeScript style) are owned by [`rules/conventions.md`](conventions.md); this file owns folder shape and placement. Apply both when adding files.
