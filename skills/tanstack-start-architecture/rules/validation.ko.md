...
- project-structure guidance가 `src/routes`, custom `routesDirectory`, generated `routeTree.gen.ts`, shared nested folders를 다룸.
- `src/lib`, `src/features` 같은 shared nested folders는 official TanStack law가 아니라 Hypercore/repo-local convention으로 label됨.
...
- "TanStack Start folder structure를 검토하고 nested `src/lib` grouping을 허용해줘."
- "TanStack Start에서 `src/features/billing`과 `src/routes/billing` 구조가 맞는지 봐줘."
