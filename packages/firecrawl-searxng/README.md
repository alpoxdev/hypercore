# Firecrawl + SearXNG + Jina Reader Self-Hosted

Firecrawl (웹 스크래핑/크롤링) + SearXNG (메타검색 엔진) + Jina Reader (LLM 친화적 웹 리더) 셀프호스팅 Docker Compose 패키지.
Claude Code MCP 서버와 연동하여 로컬에서 웹 검색 + 크롤링 + LLM 최적화 인프라를 제공합니다.
...
| jina-reader | ghcr.io/intergalacticalvariable/reader | 40997 | LLM 친화적 웹 리더 |
...
# Jina Reader 테스트 (LLM 친화적 출력)
curl "http://localhost:40997/https://example.com"
```
...
# Jina Reader MCP (셀프호스팅)
claude mcp add jina-reader \
  -e JINA_READER_URL=http://localhost:40997 \
  -- npx -y mcp-jinaai-reader
...
| `JINA_READER_PORT` | 40997 | Jina Reader 외부 포트 |
| `JINA_READER_ENDPOINT` | http://jina-reader:8080 | Jina Reader 내부 엔드포인트 |
...
서비스 로그 확인:
```bash
docker compose logs jina-reader
```
...
- [Jina Reader 공식 문서](https://jina.ai/reader)
- [mcp-jinaai-reader NPM](https://www.npmjs.com/package/mcp-jinaai-reader)
