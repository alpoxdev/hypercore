...
| `src/lib`, `src/features`, `src/services`, `src/db`, `src/server`, `src/config` 같은 shared folders | Hypercore convention | ownership과 boundaries를 명확히 할 때 touched code에 적용 |
...
Routes를 가로질러 공유되는 code는 route root 밖에 두고 nested grouping을 사용할 수 있습니다. Nested grouping이 domain ownership, runtime boundary, dependency direction을 더 잘 보여준다면 flat `src/lib/*.ts` layout을 기본값으로 강요하지 않습니다.
...
작은 project는 더 단순하다면 shared folder를 flat하게 유지할 수 있습니다. 이 선택은 official TanStack requirement가 아니라 Hypercore 또는 repo-local convention으로 label합니다.
...
| Shared `src/lib` / `src/features` layout을 official TanStack law처럼 제시 | 차단 |
...
- [ ] Shared folders는 boundaries를 명확히 할 때 nested `src/lib`, `src/features`, `src/services`, `src/db`, `src/server`, `src/config` grouping을 사용할 수 있음.
