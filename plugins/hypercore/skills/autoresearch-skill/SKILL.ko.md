...
- run contract, 출처 정책, reset 이벤트, trace assertion은 [rules/context-sourcing-and-trace.md](rules/context-sourcing-and-trace.md)를 사용한다.
...
- upstream의 Verify/Guard, git memory, crash recovery, result log status를 적용할 때는 [references/upstream-autoresearch-patterns.md](references/upstream-autoresearch-patterns.md)를 사용한다.
- 한국어 최종 보고, 점수 상승 설명, 변경 귀속, 대시보드 표시 요약은 [references/reporting-and-score-explanation.ko.md](references/reporting-and-score-explanation.ko.md)를 사용한다.
...
- 사람이 읽는 `description`, 점수 근거, 점수 변화 설명, changelog 항목, 대시보드 문구는 사용자가 다른 언어를 명시하지 않는 한 한국어로 유지한다.

아티팩트 생명주기 요구사항:

- `.hypercore/autoresearch-skill/[skill-name]/` 아래에 워크스페이스를 만든다
- 원본 대상 스킬은 `SKILL.md.baseline`으로 저장하고, support file도 바뀔 수 있으면 `baseline-files.json` 또는 `baseline/` snapshot을 만든다
- 매 실험 뒤 `results.tsv`와 `results.json`을 동기화한다
- prompt pack, eval set, target files, environment, rollback conditions, guard policy, source policy, trace assertions를 artifacts에 기록한다
...
- 대상 `SKILL.md` 전체를 읽는다.
- 변이 전에 intent, scope, authority, evidence, tools, output, verification, stop condition으로 run contract를 기록한다.
- 직접 연결된 `rules/`와 `references/`를 읽는다.
- 실패가 동작 문제인지, 구조 문제인지, 둘 다인지 식별한다.
- `SKILL.md.baseline`을 저장하고 support file이 scope에 있으면 함께 snapshot한다.

#### Phase 1: Eval 세트 구성

- 성공 조건을 이진 pass/fail 체크로 바꾼다.
- scoring method를 dry-run하고 parse 가능하고 반복 가능한 점수가 아니면 거부한다.
- 외부 근거, 도구, delegation이 correctness에 영향을 주면 source-sensitive 또는 trace-based check를 추가한다.
- eval은 서로 겹치지 않고 관찰 가능하며 속이기 어려워야 한다.
- 대상이 스킬이면 문체가 아니라 트리거나 구조 품질을 확인하는 eval을 최소 하나 포함한다.
- Verify score and Guard checks are separate: Verify는 score movement를 측정하고, Guard는 trigger boundaries, artifact schema, support links, safety regression을 차단한다.
- Do not change the eval set to make a mutation pass. eval set이 틀렸다면 새 score 비교 전에 reset event를 기록한다.
...
- 최근 `results.tsv`, `results.json`, `changelog.md`, 선택적 git experiment history를 먼저 읽는다.
- 버린 가설 반복을 피하고 하나의 가설과 한 문장 mutation description을 편집 전에 쓴다.
- 하나의 변이만 적용한다.
- 같은 eval 세트와 guard check를 다시 돌린다.
- 점수가 오르고 guard가 통과하면 유지한다. 점수가 같거나 나빠지거나 guard가 실패하거나 무회귀 단순화 증거 없이 복잡성이 늘면 버린다.
- discard, crash, no-op, hook-blocked, metric-error까지 모든 시도를 기록한다.
- 유지 또는 재작업 유지된 변이마다 어느 eval 영역이 개선됐는지, 점수가 얼마나 움직였는지, 어떤 파일을 바꿨는지, 그 변화의 근거가 무엇인지 기록한다.
...
- 최종 보고는 한국어로 작성하고 baseline 점수, 최종/최고 점수, 정확한 delta, 어느 eval/category에서 점수가 올랐는지, 어떤 파일을 수정했는지, 각 변경을 왜 유지했는지, dashboard 경로, caveat를 포함한다.
...
- `.hypercore/autoresearch-skill/[skill-name]/score-explanation.md`: 한국어 점수 변화, eval/category별 상승 근거, 파일별 변경 이유
- `.hypercore/autoresearch-skill/[skill-name]/final-report.md`: 사용자에게 보여 줄 한국어 최종 요약
...
- 실행이 외부/current source, 도구, delegation을 쓰면 `run-contract.md`, `source-ledger.md`, 또는 `trace-summary.md`
